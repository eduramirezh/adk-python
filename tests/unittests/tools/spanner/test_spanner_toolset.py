...
async def test_spanner_toolset_tools_default():
  ...
  assert isinstance(toolset._tool_config, SpannerToolConfig)  # pylint: disable=protected-access
  assert toolset._tool_config.__dict__ == SpannerToolConfig().__dict__  # pylint: disable=protected-access
...
async def test_spanner_toolset_selective(selected_tools):
  ...
  toolset = SpannerToolset(
      ...
      spanner_tool_config=SpannerToolConfig(),
  )
  tools = await toolset.get_tools()
  assert tools is not None
...
async def test_spanner_toolset_without_read_capability(
  ...
  spanner_tool_config = SpannerToolConfig(capabilities=[])
  toolset = SpannerToolset(
      credentials_config=credentials_config,
      tool_filter=selected_tools,
      spanner_tool_config=spanner_tool_config,
  )
