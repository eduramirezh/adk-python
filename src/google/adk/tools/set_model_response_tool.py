...
class SetModelResponseTool(BaseTool):
  ...
  def __init__(self, output_schema: type[BaseModel]):
    ...
    def set_model_response(**kwargs) -> str:
  ...
  async def run_async(
    ...
    """Process the model's response and mark it for extraction.
    ...
    Returns:
      Confirmation message that the response was set.
    """
    # Validate the input matches the expected schema
    validated_response = self.output_schema.model_validate(args)

    # Store the validated response in the tool context for later extraction
    # The flow will detect this special marker and extract the content
    tool_context._invocation_context.session.state[MODEL_JSON_RESPONSE_KEY] = (
